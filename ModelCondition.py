...
class AttnBlock(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        self.group_norm = nn.GroupNorm(1, in_ch)
    ...
    def forward(self, x):
        ...
        q = q.permute(0, 2, 1).view(B, H * 1, C)
        k = k.view(B, C, H * 1)
        w = torch.bmm(q, k) * (int(C) ** (-0.5))
        assert list(w.shape) == [B, H * 1, H * 1]
        w = F.softmax(w, dim=-1)

        v = v.permute(0, 2, 1).view(B, H * 1, C)
        h = torch.bmm(w, v)
        assert list(h.shape) == [B, H * 1, C]
        h = h.view(B, H, C).permute(0, 2, 1)
        h = self.proj(h)
...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, tdim, dropout, attn=True):
        ...
        self.block1 = nn.Sequential(
            ...
            Swish(),
            nn.Conv1d(in_ch, out_ch, 5, stride=1, padding=2),
        )
...
class UNet(nn.Module):
    def __init__(self, T, num_labels, ch, ch_mult, num_res_blocks, dropout):
        ...
        self.head = nn.Conv1d(2, ch, kernel_size=5, stride=1, padding=2)
        ...
        self.tail = nn.Sequential(
            nn.GroupNorm(1, now_ch),
            Swish(),
            nn.Conv1d(now_ch, 2, 5, stride=1, padding=2)
        )
...
if __name__ == '__main__':
    batch_size = 20
    model = UNet(
        T=1000, num_labels=6, ch=32, ch_mult=[1, 2, 3, 4],
        num_res_blocks=2, dropout=0.1)
    x = torch.randn(batch_size, 2, 1000)
    t = torch.randint(1000, size=[batch_size])
    labels = torch.randint(6, size=[batch_size])
